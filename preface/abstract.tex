...
automatically. The rules are physically interpretable: an operator can read them and
verify that they match known turbine behaviour. The second study then asks whether
accuracy can be pushed higher by methods that work directly with continuous sensor
values rather than with the discretized bins that rule-based classifiers require. It
compares deep reinforcement learning (DRL) agents (DQN, PPO, A2C) with supervised
machine learning classifiers (Random Forest, Gradient Boosting, MLP) on the same task.
